...
# -- 2010 -> use CFSR
# 2012+ -> use CFS
# 2011 is weird, RDA format is like CFSR through Apr 1, then CFS after (all in ds094)
...
# CFS
for f in ~/rda/ds094.0/${YEAR}/cdas1.${YEAR}????.pgrbh.tar .
do
 THISDATE="$(echo $(basename $f) | cut -d'.' -f2)"
 tar -xvf "$f" --wildcards "*.pgrbhanl.grib2"
 mv cdas1.t00z.pgrbhanl.grib2 pgbhnl.gdas.${THISDATE}00.grb2
 mv cdas1.t06z.pgrbhanl.grib2 pgbhnl.gdas.${THISDATE}06.grb2
 mv cdas1.t12z.pgrbhanl.grib2 pgbhnl.gdas.${THISDATE}12.grb2
 mv cdas1.t18z.pgrbhanl.grib2 pgbhnl.gdas.${THISDATE}18.grb2
done
